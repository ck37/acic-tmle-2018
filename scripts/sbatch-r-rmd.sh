...
#
# Wall clock limit (override with command-line -t option in Makefile):
...
# Login nodes contain "brc", compute nodes contain "savio".
if [[ $(uname -n) == *"brc"* ]]; then
  # We're on Savio.
  hpc_system="savio"
  ...
  # We're on XSEDE bridges.
  hpc_system="bridges"

  # bridges version (3.4.1-mkl):
  module load R
...
if [ $make_rmd == 1 ]; then
  ...
  # Use file output format that is consistent with Jade's analyses.
